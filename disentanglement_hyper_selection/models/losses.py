...
class BtcvaeLoss():
    ...
    def __call__(self, data, recon_batch, latent_dist, is_train, storer, wtp_pred, wtp,
        ...
        # print(anneal_reg)
        # APPROACH 1
        loss =  rec_loss + (self.alpha * mi_loss + self.beta * tc_loss + self.gamma * dw_kl_loss) + anneal_reg * self.delta * mse_loss

        # kl_loss = _kl_normal_loss(*latent_dist, storer)

        return loss, rec_loss, self.alpha * mi_loss, self.beta * tc_loss, self.gamma * dw_kl_loss, anneal_reg * self.delta * mse_loss, rsq, latent_kl
