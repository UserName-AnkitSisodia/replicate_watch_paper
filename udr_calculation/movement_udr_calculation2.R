...
udr_beta_inner <- function(beta,seed,m) {
  focal_df <- read.csv(paste0("movement_s",seed,"b",beta,"m",m,"_mean_params_test2.csv"))
  ...
  for(i in 1:5)
  {
    if(i!=seed)
    {
      comparison_df <- read.csv(paste0("movement_s",i,"b",beta,"m",m,"_mean_params_test2.csv"))
