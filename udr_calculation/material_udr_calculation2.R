...
udr_beta_inner <- function(beta,seed,m) {
  focal_df <- read.csv(paste0("material_s",seed,"b",beta,"m",m,"_mean_params_test2.csv"))
  ...
  for(i in 1:10)
  {
    if(i!=seed)
    {
      comparison_df <- read.csv(paste0("material_s",i,"b",beta,"m",m,"_mean_params_test2.csv"))
...
write.csv(df_udr,"df_udr2.csv",row.names=FALSE)
